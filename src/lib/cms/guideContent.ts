    ...
    sections: [
      {
        ...
        steps: [
          ...
          p("جديد — معاينة حيّة بجوار كل قسم: بجانب الحقول تظهر لوحة «كما يظهر على الموقع» تعرض القسم بتصميمه الفعلي (الألوان والأزرار والشارات والصور). وأثناء كتابتك في أي حقل تتحدّث المعاينة فورًا حرفًا بحرف — فترى الشكل النهائي قبل الضغط على «حفظ».", "New — a live preview beside every section: next to the fields a “As shown on the site” panel shows the section in its real design (colors, buttons, badges, images). As you type in any field, the preview updates instantly, letter by letter — so you see the final look before clicking “Save”."),
