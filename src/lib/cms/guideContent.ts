...
export const GUIDE: GuidePart[] = [
  ...
  // ==================== الجزء 2: التحكّم في المحتوى والعناصر ====================
  {
    ...
    sections: [
      ...
      {
        id: "section-view",
        title_ar: "المعاينة الحيّة بجانب الحقول (Section View)",
        title_en: "Live Preview Beside the Fields (Section View)",
        intro_ar: "أغلب المحرّرات صار لها «معاينة حيّة»: الشاشة مقسومة عمودين — الحقول على جهة، وصورة طبق الأصل من القسم كما سيظهر على الموقع على الجهة المقابلة — وتتحدّث فورًا مع كل حرف تكتبه.",
        ...
        steps: [
          ...
          p("على شاشة الموبايل الصغيرة يظهر العمودان فوق بعضهما بدل جنبٍ إلى جنب — الحقول أولًا ثم المعاينة تحتها.", "On a small mobile screen the two columns stack instead of sitting side by side — the fields first, then the preview below."),
        ],
      },
      {
        id: "titles",
      ...
      },
      {
        ...
        steps: [
          cms("detail-image", "في المحرّر اضغط منطقة الصورة أو زر «رفع صورة»، ثم اختر الملف من جهازك. ولتعديل صورة موجودة اضغط «تعديل الصورة».", "In the editor click the image area or the “Upload image” button, then choose the file from your device. To edit an existing image, click “Edit Image”."),
          ...
          cms("image-editor", "محرّر الصور: بجانب كل صورة زر «تعديل الصورة» يفتح محرّرًا كاملًا فيه معاينة حيّة للصورة تتغيّر مع كل تعديل. يمكنك:", "Image editor: next to each image a “Edit Image” button opens a full editor with a live preview of the image that updates as you tweak. You can:", undefined, undefined, [
            ...
            ["التكبير (Zoom) والتدوير (Rotation) لضبط الميلان أو الاتجاه.", "Zoom and rotate to fix tilt or orientation."],
            ["ضبط السطوع والتباين والتشبّع — والتأثير يظهر على الصورة لحظيًا.", "Adjust brightness, contrast, and saturation — the effect shows on the image live."],
            ["تحديد عرض الإخراج (تصغير أبعاد الصورة النهائية عند الحاجة).", "Set the output width (shrink the final image's dimensions when needed)."],
            ["ثم «اعتماد» ليُحفظ التعديل ويُرفع تلقائيًا. المحرّر متاح لكل صور اللوحة، وفي المعارض لكل صورة زر «تعديل ✏️» خاص بها.", "Then “Apply” to save the edit and upload it automatically. The editor is available for every image in the panel, and in galleries each photo has its own “Edit ✏️” button."],
          ]),
          clip("image-editor", "شاهد المحرّر متحركًا: كل ما تحرّك مؤشّر التشبّع أو السطوع، يتغيّر تأثير الصورة على الفور في المعاينة.", "See the editor in motion: as you drag the saturation or brightness slider, the image effect changes instantly in the preview."),
          clip("image-workflow", "العملية كاملة من أولها لآخرها: فتح المحرّر ← قصّ الصورة يدويًا وتعديل ألوانها ← «اعتماد» ← «حفظ التعديلات» ← تظهر الصورة المعدّلة على صفحة الموقع الحقيقية بعد التحديث.", "The whole process end-to-end: open the editor → crop the image freely and adjust its colours → “Apply” → “Save Changes” → the edited image appears on the real site page after a refresh."),
          ...
          p("الحد الأقصى لحجم الصورة 50 ميجابايت — ارفع صورك عالية الجودة بلا قلق. النظام يولّد تلقائيًا نسخة ويب سريعة ومحسّنة (بلا فرق تراه بعينك) ويحتفظ بالأصل الكامل، فلا حاجة لضغط الصور بنفسك قبل الرفع.", "Maximum image size is 50 MB — upload your high-quality images without worry. The system automatically generates a fast, optimized web version (with no visible quality difference) and keeps the full original, so there's no need to compress images yourself before uploading."),
          cms("branch-gallery", "المعرض (Gallery) يقبل عدة صور لنفس العنصر — أضف أو احذف أو رتّب صوره بنفس الطريقة. ومعرض صور الفرع يقبل أيضًا مقاطع فيديو مع الصور (حتى 50 ميجا للفيديو) — ارفعها بنفس زر الإضافة.", "A gallery accepts multiple images for one item — add, remove, or reorder its photos the same way. The branch photo gallery also accepts videos alongside images (up to 50 MB per video) — upload them with the same Add button."),
        ],
      },
      {
        id: "cards-lists",
      ...
      },
      {
        ...
        steps: [
          ...
          clip("text-workflow", "مثال كامل من التعديل إلى الموقع: تكتب في الحقل فتتغيّر «المعاينة الحيّة» لحظيًا ← «حفظ التعديلات» ← يظهر تعديلك على صفحة الموقع الحقيقية بعد التحديث.", "A complete example from edit to live site: you type in the field and the “live preview” updates instantly → “Save Changes” → your edit appears on the real site page after a refresh."),
  ...
  // ==================== الجزء 3: الإعدادات والطلبات ====================
  {
    id: "admin",
    title_ar: "إعدادات الموقع والطلبات",
    title_en: "Site Settings & Requests",
    sections: [
      {
        id: "settings",
    ...
    sections: [
      ...
      },
      {
        ...
        steps: [
          ...
          cms("list-branches", "تُدار من [صفحة «مراكزنا (الفروع)»](/cms/content/branches). محرّر كل فرع مقسّم لأقسام واضحة، وكل قسم يتحكّم في جزء مستقل من صفحة الفرع (لكل فرع محتواه الخاص، لا شيء مشترك بين الفروع). والأقسام هي:", "Managed from [the “Our Centers (Branches)” page](/cms/content/branches). Each branch's editor is split into clear sections, each controlling a separate part of that branch's page (every branch has its own content — nothing is shared between branches). The sections are:", undefined, undefined, [
            ...
            ["«قصص النجاح» — العنوان والوصف + اختيار قصص «أبطال عبور» التي تظهر لهذا الفرع + إضافة قصص خاصة بالفرع.", "“Success stories” — heading & text + choosing which “Oboor Champions” stories show for this branch + adding branch-specific stories."],
            ["«ملف الفرع (PDF)» — محتوى الملف القابل للتنزيل.", "“Branch profile (PDF)” — the downloadable file's content."],
            ["«معرض صور الفرع» — صور وفيديوهات الفرع.", "“Branch photo gallery” — the branch's photos and videos."],
          ]),
      ...
      },
      {
        ...
        steps: [
          ...
          p("ربط القصة بفرع: أي قصة تضيفها هنا يمكن اختيارها لتظهر داخل صفحة فرع معيّن — من محرّر ذلك الفرع → قسم «قصص النجاح» (مشروح في قسم «مراكزنا (الفروع)»).", "Linking a story to a branch: any story you add here can be selected to appear inside a specific branch's page — from that branch's editor → the “Success stories” section (explained in “Our Centers (Branches)”)."),
        ],
      },
      {
        id: "site-news",
    ...
    sections: [
      {
        ...
        steps: [
          ...
          p("✅ اجعل العناوين قصيرة وواضحة. وارفع صورًا واضحة عالية الجودة (حتى 50 ميجا) — النظام يحسّنها للويب تلقائيًا ويحتفظ بالأصل، ويمكنك قصّها وضبطها من «محرّر الصور».", "✅ Keep titles short and clear. Upload clear, high-quality images (up to 50 MB) — the system optimizes them for the web automatically and keeps the original, and you can crop and adjust them from the “Image editor”."),
      ...
      },
      {
        ...
        faq: [
          ...
          faq("ما هي «المعاينة الحيّة» التي تظهر بجانب الحقول؟",
              "What is the “live preview” next to the fields?",
              "صورة طبق الأصل من القسم كما سيظهر على الموقع، تتحدّث مع كل حرف تكتبه — للعرض فقط، وتكتب أنت في الحقول ثم تضغط «حفظ». (تفاصيل في قسم «المعاينة الحيّة بجانب الحقول».)",
              "A faithful preview of the section as it will look on the site, updating as you type — it's view-only; you type in the fields then click “Save”. (Details in “Live Preview Beside the Fields”.)"),
          faq("كيف أقصّ صورة أو أعدّل سطوعها وأبعادها؟",
              "How do I crop an image or adjust its brightness and dimensions?",
              "بزر «تعديل الصورة» بجانب أي صورة — يفتح محرّرًا فيه قصّ وأبعاد جاهزة وتكبير وتدوير وسطوع/تباين/تشبّع وعرض إخراج، بمعاينة حيّة، ثم «اعتماد». متاح لكل صور اللوحة.",
              "Use the “Edit Image” button next to any image — it opens an editor with crop, ready ratios, zoom, rotate, brightness/contrast/saturation, and output width, with a live preview, then “Apply”. Available for every image in the panel."),
          faq("هل يمكنني رفع فيديو في معرض الفرع؟ وهل له صوت؟",
              "Can I upload a video to the branch gallery? Does it have sound?",
              "نعم، ارفعه بنفس زر إضافة الصور (حتى 50 ميجا). في المعرض يعمل تلقائيًا بلا صوت وأول ما يخلص ينتقل للتالي؛ وفي العرض المكبّر يمكن للزائر تشغيل الصوت.",
              "Yes, upload it with the same Add button as photos (up to 50 MB). In the gallery it plays automatically muted and moves to the next when it ends; in the enlarged viewer the visitor can turn the sound on."),
          faq("هل رفع صورة كبيرة (بضع عشرات الميجابايت) سيُبطئ الموقع؟",
              "Will uploading a large image (tens of MB) slow the site down?",
              "لا — النظام يولّد تلقائيًا نسخة ويب سريعة ومحسّنة يراها الزائر، ويحتفظ بالأصل الكامل عندك. ارفع بأعلى جودة بلا قلق.",
              "No — the system automatically generates a fast, optimized web version for visitors and keeps your full original. Upload at the highest quality without worry."),
          faq("كيف أجعل قصة نجاح تظهر داخل صفحة فرع معيّن؟",
              "How do I make a success story appear inside a specific branch's page?",
              "من محرّر الفرع → قسم «قصص النجاح»: علّم على قصص «أبطال عبور» لتظهر بطاقاتها، أو أضف قصة خاصة بالفرع وحده — وتظهر مباشرةً في المعاينة الحيّة.",
              "From the branch editor → the “Success stories” section: tick “Oboor Champions” stories to show their cards, or add a story for that branch only — it shows immediately in the live preview."),
      ...
      },
      {
        ...
        glossary: [
          ...
          term("المعاينة الحيّة (Section View)", "Live Preview (Section View)", "عمود بجانب الحقول يعرض القسم طبق الأصل كما سيظهر على الموقع، ويتحدّث لحظيًا مع الكتابة — للعرض فقط.", "A column beside the fields showing the section exactly as it'll look on the site, updating live as you type — view-only."),
          term("محرّر الصور", "Image Editor", "أداة تفتح بزر «تعديل الصورة» للقصّ وضبط الأبعاد والتكبير والتدوير والسطوع/التباين/التشبّع بمعاينة حيّة.", "A tool opened by the “Edit Image” button for cropping, sizing, zooming, rotating, and brightness/contrast/saturation with a live preview."),
