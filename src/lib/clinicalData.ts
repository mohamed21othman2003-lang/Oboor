// بيانات صفحات تفاصيل الخدمات العيادية (مستخرجة من فيجما)
...
const AGE_PROGRAMS = (adultSub: string): ClinicalBlock => ({
  ...
  adult: { title: "برنامج للكبار", sub: adultSub, label: "برنامج متكامل للكبار" },
...
const AGE_PROGRAMS_EN = (adultSub: string): ClinicalBlock => ({
  kind: "agePrograms",
  heading: "Programs Offered",
  adult: { title: "Adult Program", sub: adultSub, label: "Comprehensive program for adults" },
...
export const CLINICAL_SERVICES_EN: ClinicalService[] = [
  {
    slug: "physical",
...
export const getClinicalService = (slug: string, locale: Locale = "ar") =>
  (locale === "en" ? CLINICAL_SERVICES_EN : CLINICAL_SERVICES).find((s) => s.slug === slug);
