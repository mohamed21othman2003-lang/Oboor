...
export default function SubmissionsTable({
  ...
  const condOpts = useMemo(() => [{ value: "", label: t("كل أنواع الحالات", "All case types") }, ...caseTypeFilterOptions(items.map((it) => v(it, "case_type")), en)], [items, en]);
  const sortOpts = [
    { value: "newest", label: t("الأحدث أولاً", "Newest first") },
    { value: "oldest", label: t("الأقدم أولاً", "Oldest first") },
    { value: "alpha", label: t("أبجديًا (ولي الأمر)", "Alphabetical (parent)") },
    { value: "branch", label: t("حسب الفرع", "By branch") },
    { value: "case", label: t("حسب نوع الحالة", "By case type") },
  ];
  const rangeOpts = [
  ...
  const sorted = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "alpha": arr.sort((a, b) => v(a, "parent_name").localeCompare(v(b, "parent_name"), coll)); break;
      case "branch": arr.sort((a, b) => v(a, "branch").localeCompare(v(b, "branch"), coll) || time(b) - time(a)); break;
      case "case": arr.sort((a, b) => v(a, "case_type").localeCompare(v(b, "case_type"), coll) || time(b) - time(a)); break;
  ...
  const resetFilters = () => { setQuery(""); setBranch(""); setCondition(""); setRange(""); setSort("newest"); setPage(1); };
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-line bg-white p-3 shadow-sm">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <span className="flex items-center gap-1.5 text-ink-soft">{I.sort}<div className="w-[180px]"><CustomSelect value={sort} onChange={setSort} options={sortOpts} placeholder={t("ترتيب", "Sort")} /></div></span>
