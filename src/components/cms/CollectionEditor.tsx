...
// عارض صورة بالحجم الكامل — يُرسَل عبر Portal إلى body حتى لا يتأثّر بأي عنصر أب
// له transform/filter (الذي يجعل fixed نسبةً إليه لا للشاشة فتظهر الصورة صغيرة).
function Lightbox({ src, onClose }: { src: string; onClose: () => void }) {
  const { lang } = useCmsLang();
  const t = (ar: string, e: string) => (lang === "en" ? e : ar);
  useEffect(() => {
...
function ImageInput({ f, value, pathFallback, type, id, isNew, onUploaded, onChange }: { f: FieldSchema; value: unknown; pathFallback: string; type: string; id: string; isNew: boolean; onUploaded: (it: CmsItem) => void; onChange: (v: string) => void }) {
  ...
  function onPick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    e.target.value = "";
    if (!file) return;
    if (file.size > 50 * 1024 * 1024) { setErr(t("الحد الأقصى 50 ميجابايت.", "Maximum size is 50 MB.")); return; }
    setErr("");
    setPending(file);
  }
