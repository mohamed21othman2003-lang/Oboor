  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <StatCounter icon={I.inbox} value={counts.total} label={t("إجمالي الرسائل", "Total messages")} active={range === ""} onClick={() => setRange("")} hint={t("عرض الكل", "Show all")} />
        <StatCounter icon={I.chats} value={counts.week} label={t("آخر ٧ أيام", "Last 7 days")} active={range === "7"} onClick={() => setRange("7")} hint={t("فلترة", "Filter")} />
        <StatCounter icon={I.mail} value={counts.today} label={t("رسائل اليوم", "Today's messages")} active={range === "today"} onClick={() => setRange("today")} hint={t("فلترة", "Filter")} />
      </div>
