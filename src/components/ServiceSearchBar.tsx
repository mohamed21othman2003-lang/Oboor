...
function SelectField({
  ...
  return (
    <label className="flex flex-1 cursor-pointer items-center justify-start gap-3 px-3 py-1">
      {boxed ? (
      ...
      <div className="min-w-0 flex-1 text-start">
        ...
        <select
          ...
          className="w-full cursor-pointer appearance-none truncate bg-transparent text-sm font-bold text-ink focus:outline-none"
        >
          {children}
        </select>
      </div>
...
export default function ServiceSearchBar({ locale = "ar", searchLabel }: { regionValue?: string; locale?: Locale; searchLabel?: string }) {
  ...
  const programOptions = region === ALL ? category.items : category.items.filter((i) => i.regions?.includes(region));
  ...
  function search() {
    const picked = programIdx !== ALL ? programOptions[Number(programIdx)] : null;
    if (picked) {
  ...
  return (
    <div className="flex flex-col items-stretch gap-2 rounded-2xl bg-white p-3 shadow-md ring-1 ring-line lg:flex-row lg:items-center">
      ...
      >
        <option value={ALL}>{pick(locale, "كل البرامج", "All Programs")}</option>
        {programOptions.map((p, i) => <option key={p.title} value={i}>{p.title}</option>)}
      </SelectField>
      ...
      >
        <option value={ALL}>{pick(locale, "كل المناطق", "All Regions")}</option>
