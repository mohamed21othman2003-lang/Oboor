...
HIDDEN_PER_MODEL = {
    ...
    Branch: {
        # «الحي» غير مستخدم في بيانات الفروع الفعلية (العنوان الكامل يكفي) ⇒ نخفيه من المحرّر
        "area_ar", "area_en",
        # الإحداثيات (lat/lng) مكرّرة مع «رابط الموقع على الخريطة» الذي يغطّي الاتجاهات
        # لكل الفروع؛ نخفيها لتفادي حقلين للموقع (تبقى في البيانات لمن رفعها سابقاً).
        "lat", "lng",
        # ساعات العمل والتقييمات فارغة في كل الفروع ولا تُستخدم ⇒ نخفيها لتقليل التشويش
        # (تظل في الموديل وتظهر تلقائياً لو مُلئت مستقبلاً).
        "hours_ar", "hours_en", "rating", "reviews_count",
    },
    SiteSettings: {
